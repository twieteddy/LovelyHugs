...
public class HugCommand implements CommandExecutor {
  ...
  private static final String MSG_YOU_HUGGED =
      ChatColor.LIGHT_PURPLE
          ...
          + ChatColor.LIGHT_PURPLE
          + "umarmt";
  private static final String MSG_HUGGED_BY =
      ChatColor.LIGHT_PURPLE
          + "Du wurdest von"
          + ChatColor.YELLOW
          + " %s "
          + ChatColor.LIGHT_PURPLE
          + "umarmt";
  private static final String MSG_NO_PLAYER_NEARBY =
      ChatColor.LIGHT_PURPLE + "Es ist leider niemand in der Nähe =(";
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    ...
    if (!sender.hasPermission(PERM_HUG)) {
      return false;
    }

    // Get all nearby players within the hug distance
    Player player = (Player) sender;
    Player closest = getClosestPlayer(player);
  ...
  private void hugPlayer(Player huggingPlayer, Player huggedPlayer) {
    ...
    huggingPlayer
        .getWorld()
        .spawnParticle(Particle.HEART, huggingPlayer.getLocation(), 64, 0.5d, 1.0d, 0.5d);
    huggedPlayer
        .getWorld()
        .spawnParticle(Particle.HEART, huggedPlayer.getLocation(), 64, 0.5d, 1.0d, 0.5d);
  ...
  private Player getClosestPlayer(Player player) {
    List<Entity> nearbyPlayers =
        player
            .getNearbyEntities(
                HugCommand.HUG_DISTANCE, HugCommand.HUG_DISTANCE, HugCommand.HUG_DISTANCE)
            .stream()
    ...
    Entity closest =
        Collections.min(
            nearbyPlayers,
            Comparator.comparing(e -> e.getLocation().distanceSquared(player.getLocation())));
